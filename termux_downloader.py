...
def choose_media_type():
    console.print("[bold yellow]Select media type:[/bold yellow]")
    console.print("1. Video\n2. Audio")
    media_choice = Prompt.ask("Enter your choice", choices=["1", "2"])
    return "video" if media_choice == "1" else "audio"

def choose_save_location():
    console.print("[bold yellow]Choose where to save the file:[/bold yellow]")
    console.print("1. Downloads folder\n2. Videos folder\n3. Custom folder")
    location_choice = Prompt.ask("Enter your choice", choices=["1", "2", "3"])
...
def run_yt_dlp_audio(url, output_path):
    command = [
...
def run_yt_dlp_video(url, output_path):
    command = [
...
def clear_cache():
    ...
    os.system("rm -rf ~/.cache/yt-dlp")
    os.system("rm -rf ~/.spotdl")
